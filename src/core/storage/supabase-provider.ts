...
export class SupabaseProvider implements StorageProvider {
  private local: SQLiteProvider;
  private supabase: SupabaseClient | null = null;
  private syncQueue: ConversationItem[] = [];
  private isSyncing = false;
  private projectIdCache: Record<string, string> = {}; // path -> uuid

  constructor(localProvider: SQLiteProvider, supabaseUrl?: string, supabaseKey?: string) {
  ...
  private async processSyncQueue() {
    if (this.isSyncing || this.syncQueue.length === 0 || !this.supabase) return;
  ...
  private async syncMemoryState(projectPath: string, gitBranch: string, state: MemoryState) {
  ...
  private async ensureProjectExists(path: string): Promise<string> {
    if (this.projectIdCache[path]) return this.projectIdCache[path];
